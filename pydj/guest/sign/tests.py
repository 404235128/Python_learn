...
from django.test import Client#Client()类提供的get()和post()方法可以模拟GET/POST请求
...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=1,name='oneplus 3 event',
                             status=True,limit=2000,address='shenzhen',start_time='2019-07-08 02:18:22')
...
class IndexPageTest(TestCase):
    ...
    def test_index_page_renders_index_template(self):
        ...
        self.assertTemplateUsed(response,'index.html')#断言是否用的index.html模板响应
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_success(self):
        '''登录成功'''
        test_data = {'username': 'luwh', 'password': '1234'}
        response = self.c.post('/login_action/', data=test_data)
...
class EventManageTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(id=2,name='xiaomi5',limit=2000,status=True,
                             address='beijing',start_time=datetime(2019,7,10,14,0,0))
        self.c=Client()
...
class GuestManageTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(id=1,name='xiaomi5',limit=2000,address='beijing',
                             status=1,start_time=datetime(2019,7,10,14,0,0))
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(id=1,name='xiaomi5',limit=2000,address='beijing',
                             status=1,start_time='2019-7-10 12:30:00')
        Event.objects.create(id=2,name='oneplus4',limit=2000,address='shenzhen',
                             status=1,start_time='2019-7-14 12:30:00')
        Guest.objects.create(realname='alen',phone=18611001100,email='alen@mail.com',
                             sign=0,event_id=1)
